...
// Function to sort arr[] of
// size n using bucket sort
std::vector<float> bucketSort_2(std::vector<float> arr, int number_of_threads, int buckets_quan)
{
    // 1) Create n empty buckets
    std::vector<std::vector<float>> b(buckets_quan);
 
    // 2) Put array elements
    // in different buckets
    auto bucket_split_start = std::chrono::high_resolution_clock::now();
    std::vector<std::vector<float>> thread_bucket(buckets_quan);
    std::vector<std::vector<std::vector<float>>> all_thread_buckets(number_of_threads, thread_bucket);
    #pragma omp parallel for schedule(runtime)
    for (LLONG_UINT i = 0; i < arr.size(); ++i) {
        int bi = buckets_quan * arr[i]; // Index in bucket
